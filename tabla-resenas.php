<?php
  ...
  $sql = "SELECT id, nombre, email, mensaje FROM valoraciones";
...
<header class="header header-mini"> 
    <div class="header-title">Tabla de todos las Valoraciones</div> 
    <nav aria-label="breadcrumb">
       <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="/">Aqui se puede modificar los datos de las Valoraciones o simplemente borrarlos.</a></li>
       </ol>
    </nav>
 </header> 
    ...
    <table  class="table table-bordered" style="width: 70%; margin: 0 auto; border-collapse: collapse; border: 1px solid #ccc;">
      <thead>
        <tr>
          ...
          <th scope="col" style="border: 1px solid #ccc; padding: 8px;">MENSAJE</th>
      ...
      <tbody>
        ...
        if ($result->num_rows > 0) {
          while ($row = $result->fetch_assoc()) {
            ...
            echo "<td style='border: 1px solid #ccc; padding: 8px;'>" . $row["mensaje"] . "</td>";
            echo '<td style="border: 1px solid #ccc; padding: 8px;">
              <a href="editar-resenas.php?id=' . $row["id"] . '" class="btn btn-primary btn-sm">Editar</a>
              <a href="eliminar-resenas.php?id=' . $row["id"] . '" class="btn btn-primary btn-sm" onclick="return confirm(\'¿Estás seguro de que desea eliminar este registro?\')">Eliminar</a>
            </td>';
            echo "</tr>";
          }
        } else {
